...
```py
container = Container(validate_assignment=ValidationMode.STRICT) # Default, 1.0 behavior
...
These new features will allow users of 2.0 to have much more control of the manner in which their structures are validated, (hopefully) firmly crossing this one off the TODO list and attention can instead be focused on improving the system going forward.
...
# You can also get things like the amount of straight rails needed to place
# this train in a straight line, which is useful for RailPlanner
print(config.rail_length) # 18
```

The syntax that Draftsman uses is a superset of the community version, which allows enough flexibility to specify any arbitrary train format. See `examples/train_configuration_usage.py` for much more info about the additional features.
...
# Method correctly handles receiving a single WaitCondition object as well
