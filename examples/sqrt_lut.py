...
def main():
    ...
    for signal_name in signals.raw:
        if signal_name in {"signal-item-parameter", "signal-fuel-parameter", "signal-fluid-parameter", "signal-signal-parameter", "signal-unknown"}:
            continue
    ...
    def add_row(dc, cc):
        ...
        bp.add_circuit_connection("green", entity_1=f"cc_{num_ccs}", entity_2=f"dc_{num_ccs}")
        try:
            bp.add_circuit_connection("red", entity_1=f"dc_{num_ccs - 1}", entity_2=f"dc_{num_ccs}")
            bp.add_circuit_connection("red", entity_1=f"dc_{num_ccs - 1}", side_1="output", entity_2=f"dc_{num_ccs}", side_2="output")
        except KeyError:
            pass
    ...
    dc.conditions = [
        Input("signal-each", {"green"}) <= Input("signal-I", {"red"})
    ]
    dc.outputs = [
        Output("signal-O", copy_count_from_input=False)
    ]
