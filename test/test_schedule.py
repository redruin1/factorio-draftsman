...
class TestWaitCondition:
    def test_constructor(self):
        ...
        # Inactivity
        w = WaitCondition(WaitConditionType.INACTIVITY, compare_type=WaitConditionCompareType.AND)
    ...
    def test_to_dict(self):
        w = WaitCondition(WaitConditionType.CIRCUIT_CONDITION, condition=("signal-A", "!=", "signal-B"))
        assert w.to_dict() == {
            "type": "circuit",
            "compare_type": "or",
            "condition": {
        ...
        w = WaitCondition(WaitConditionType.CIRCUIT_CONDITION, condition=("signal-A", "<", 100))
        assert w.to_dict() == {
            "type": "circuit",
            "compare_type": "or",
            "condition": {
    ...
    def test_bitwise_and(self):
        ...
        signal_sent = WaitCondition(WaitConditionType.CIRCUIT_CONDITION, condition=("signal-A", "==", 100))
        sum1 = signal_sent & conditions
        assert isinstance(sum1, WaitConditions)
        assert len(sum1) == 3
        assert sum1 == WaitConditions([
            WaitCondition(WaitConditionType.CIRCUIT_CONDITION, condition=("signal-A", "==", 100)),
            WaitCondition(WaitConditionType.FULL_CARGO, compare_type="and"),
            WaitCondition(WaitConditionType.INACTIVITY, compare_type="and")
        ])

        # WaitCondition and error type
        with pytest.raises(ValueError, match="Can only perform this operation on <WaitCondition> or <WaitConditions> objects"):
        ...
        assert sum2 == WaitConditions([
            WaitCondition(WaitConditionType.FULL_CARGO),
            WaitCondition(WaitConditionType.INACTIVITY, compare_type="and"),
            WaitCondition(WaitConditionType.CIRCUIT_CONDITION, compare_type="and", condition=("signal-A", "==", 100)),
        ])
    ...
    def test_repr(self):
        ...
        assert repr(w) == "<WaitCondition>{type='inactivity', compare_type='or', ticks=300}"
        w = WaitCondition(WaitConditionType.ITEM_COUNT, condition=("signal-A", "=", "signal-B"))
        assert repr(w) == "<WaitCondition>{type='item_count', compare_type='or', condition=('signal-A', '=', 'signal-B')}"
...
class TestSchedule:
    ...
    def test_add_locomotive(self):
        ...
        with pytest.raises(TypeError, match="'locomotive' must be an instance of <Locomotive>"):
    ...
    def test_remove_stop(self):
        ...
        with pytest.raises(ValueError, match=re.escape("No station with name 'Station A' and conditions '<WaitConditions>[<WaitCondition>{type='full', compare_type='or'}]' found in schedule")):
            s.remove_stop("Station A", wait_conditions=full_cargo)

        # Remove stop that doesn't exist
        with pytest.raises(ValueError, match="No station with name 'Station B' found in schedule"):
        ...
        with pytest.raises(ValueError, match="No station with name 'Station A' found in schedule"):
            s.remove_stop("Station A")
