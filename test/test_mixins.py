...
class EightWayDirectionalMixinTesting(TestCase):
    def test_set_direction(self):
        rail = StraightRail()
        rail.direction = 6
        self.assertEqual(rail.direction, 6)
        rail.direction = None
        self.assertEqual(rail.direction, 0)
        with self.assertRaises(TypeError):
            rail.direction = TypeError
...
class ReadRailSignalMixinTesting(TestCase):
    def test_set_output_signals(self):
        ...
        self.assertEqual(
            rail_signal.yellow_output_signal,
            {"name": "signal-A", "type": "virtual"}
        )
        self.assertEqual(
            rail_signal.control_behavior,
        ...
        self.assertEqual(
            rail_signal.green_output_signal,
            {"name": "signal-A", "type": "virtual"}
        )
        self.assertEqual(
            rail_signal.control_behavior,
